...
{
	public static void startAction(L2PcInstance _phantom)
	{
		// TODO Do revive
		...
		if (_phantom.getPvpFlag() == 0 && (TownManager.getInstance().getTown(_phantom.getX(), _phantom.getY(), _phantom.getZ()) == null))
		{
			...
			if (_phantom.getClassId().isMage())
			{
				...
				for (L2Object npc : L2World.getObjectsAround(_phantom, 6000))
				{
				...
				if (_phantom.getTarget() != null)
				{
					if (GeoData.getInstance().canSeeTarget(_phantom, _phantom.getTarget()))
					{
						...
						if (_phantom.getClassId().getRace() == Race.orc && _phantom.getLevel() < 7)
						{
							...
							{
								_phantom.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(_phantom.getTarget().getX(), _phantom.getTarget().getY(), _phantom.getTarget().getZ(), 0));
							}
						}
						
						for (L2Skill sk : _phantom.getAtkSkills())
						{
							if (sk != null && sk.isOffensive() && !_phantom.isSkillDisabled(sk) && (sk.getWeaponDependancy(_phantom))/* && (sk.checkCondition(_phantom, _phantom.getTarget(), false))*/)
							{
								if (!_phantom.isInsideZone(L2Character.ZONE_PEACE))
								{
			...
			else
			{
				...
				if (_phantom.getTarget() != null)
				{
					...
					{
						//_phantom.getAI().moveToPawn(_phantom.getTarget(), 50);
						if (!_phantom.isMoving())
						{
							_phantom.rndWalk();
						}
		...
		if ((TownManager.getInstance().getTown(_phantom.getX(), _phantom.getY(), _phantom.getZ()) == null))
		{
			...
			if (_phantom.getClassId().isMage())
			{
				...
				for (L2Object player : L2World.getPlayersAround(_phantom, 6000))
				{
		...
		if ((TownManager.getInstance().getTown(_phantom.getX(), _phantom.getY(), _phantom.getZ()) != null))
		{
			for (L2Object gk : L2World.getVisibleObjects(_phantom, 3000))
			{
				if (gk != null && gk instanceof L2TeleporterInstance && _phantom.isInsideRadius(gk, 3000, false, true))
				{
					...
					{
						MoveToPawn sp = new MoveToPawn(_phantom, (L2TeleporterInstance) gk, L2NpcInstance.INTERACTION_DISTANCE);
						_phantom.sendPacket(sp);
						Broadcast.toKnownPlayers(_phantom, sp);
	...
	private static boolean actionDisabled(L2PcInstance phantom)
	{
	...
	public static void DoTeleportToZone(L2PcInstance _phantom)
	{
	...
	public static void GoToTown(L2PcInstance _phantom)
	{
