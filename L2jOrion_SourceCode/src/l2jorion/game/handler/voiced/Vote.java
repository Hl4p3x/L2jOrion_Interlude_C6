package l2jorion.game.handler.voiced;
...
public class Vote implements IVoicedCommandHandler
{
	...
	private static final String CREATE_VOTEDATA = "INSERT INTO accounts_voted (vote_ip, last_hop_vote, last_top_vote, last_net_vote, last_bra_vote) values(?,?,?,?,?)";
	private static final String RESTORE_VOTEDATA = "SELECT last_hop_vote, last_top_vote, last_net_vote, last_bra_vote FROM accounts_voted WHERE vote_ip=?";
	...
	public static void showHtm(L2PcInstance player)
	{
		...
		String text = HtmCache.getInstance().getHtm("data/html/mods/vote.htm");
		htm.setHtml(text);
	...
	public static void restoreVotedData(L2PcInstance player, String ip)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (rset.getLong("last_hop_vote") <= System.currentTimeMillis()){rset.getLong("last_hop_vote");}else{player.setLastHopVote(rset.getLong("last_hop_vote"));}
				if (rset.getLong("last_top_vote") <= System.currentTimeMillis()){rset.getLong("last_top_vote");}else{player.setLastTopVote(rset.getLong("last_top_vote"));}
				if (rset.getLong("last_net_vote") <= System.currentTimeMillis()){rset.getLong("last_net_vote");}else{player.setLastNetVote(rset.getLong("last_net_vote"));}
				if (rset.getLong("last_bra_vote") <= System.currentTimeMillis()){rset.getLong("last_bra_vote");}else{player.setLastBraVote(rset.getLong("last_bra_vote"));}
			}
			statement.close();
