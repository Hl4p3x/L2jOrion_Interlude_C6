...
public class PremiumMenu implements IVoicedCommandHandler, ICustomByPassHandler
{
		...
		private static final String[] VOICED_COMMANDS = { "premium", "class" };
		...
		private void showHtm(L2PcInstance player)
		{
			...
			if (player.getGlow())
			{
				htm.replace("%glow%", "<table border=0 bgcolor=ff0000><tr><td width=12 height=16></td></tr></table>");
				htm.replace("%glow1%", "OFF");
				htm.replace("%nr12%", "0");
			}
			else
			{
				htm.replace("%glow%", "<table border=0 bgcolor=00ff00><tr><td width=12 height=16></td></tr></table>");
		...
		public boolean ChangePass(L2PcInstance player, String target)
		{
				...
				try
				{
					...
					if (!((curpass == null) || (newpass == null) || (repeatnewpass == null)))
					{
						...
						{
							player.sendMessage("CurrentPass doesn't match with your current one.");
							return false;
						}
					}
					else
					{
						player.sendMessage("Invalid pass data! Format: .password CurrentPass NewPass NewPass");
						return false;
					}
				}
				catch (Exception e)
