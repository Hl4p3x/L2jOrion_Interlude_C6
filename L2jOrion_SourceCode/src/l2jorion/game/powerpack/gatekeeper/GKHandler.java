...
public class GKHandler implements IVoicedCommandHandler,ICustomByPassHandler, IBBSHandler
{
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("RB") && activeChar.isInsideZone(L2Character.ZONE_BOSS))
			msg = "Gatekeeper is not available in this area.";
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("PVP") && activeChar.isInsideZone(L2Character.ZONE_PVP))
			msg = "Gatekeeper is not available in this area.";
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("PEACE") && activeChar.isInsideZone(L2Character.ZONE_PEACE))
			msg = "Gatekeeper is not available in this area.";
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("SIEGE") && activeChar.isInsideZone(L2Character.ZONE_SIEGE))
			msg = "Gatekeeper is not available in this area.";
		...
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("TVT") && 
				activeChar._inEventTvT && TvT.is_started() )
			msg = "Gatekeeper is not available in TVT.";
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("CTF") && 
				activeChar._inEventCTF && CTF.is_started() )
			msg = "Gatekeeper is not available in CTF.";
		else if(PowerPackConfig.GLOBALGK_EXCLUDE_ON.contains("DM") && 
	...
	public void handleCommand(String command, L2PcInstance player, String parameters)
	{
		...
		if (parameters.startsWith("goto"))
		{
			try
			{
				...
				if (tpPoint != null)
				{
					...
					if (unstuckTimer > 0 && !(player.isInsideZone(L2Character.ZONE_PEACE)))
					{
		...
		else if (parameters.startsWith("formedal"))
		{
			try
			{
 				int locId = Integer.parseInt(parameters.substring(parameters.indexOf(" ") + 1).trim());
