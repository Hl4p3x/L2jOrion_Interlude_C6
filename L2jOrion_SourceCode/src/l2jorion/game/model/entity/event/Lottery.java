...
public class Lottery
{
	...
	public void increasePrize(final int count)
	{
		...
		catch (final SQLException e)
		{
			...
			LOG.warn("Lottery: Could not increase current lottery prize: " + e);
		}
		finally
		{
			CloseUtil.close(con);
	...
	private class startLottery implements Runnable
	{
		...
		public void run()
		{
			...
			catch (final SQLException e)
			{
				if (Config.ENABLE_ALL_EXCEPTIONS)
					e.printStackTrace();
				LOG.warn("Lottery: Could not restore lottery data: " + e);
			}
			finally
			{
				CloseUtil.close(con);
			...
			catch (final SQLException e)
			{
				...
				LOG.warn("Lottery: Could not store new lottery data: " + e);
			}
			finally
			{
				CloseUtil.close(con);
	...
	public int[] checkTicket(final int id, final int enchant, final int type2)
	{
		...
		try
		{
			...
			if (rset.next())
			{
				...
				if (Config.DEBUG)
				{
					LOG.warn("count: " + count + ", id: " + id + ", enchant: " + enchant + ", type2: " + type2);
				}
			}
		...
		catch (final SQLException e)
		{
			...
			LOG.warn("Lottery: Could not check lottery ticket #" + id + ": " + e);
		}
		finally
		{
			CloseUtil.close(con);
