...
public class TradeList
{
	...
	public synchronized boolean PrivateStoreSell(L2PcInstance player, ItemRequest[] items, int price)
	{
		...
		for(ItemRequest item : items)
		{
			...
			if(oldItem == null){
				if(Config.DEBUG){
					LOG.info("[PrivateStoreSell] player.checkItemManipulation(item.getObjectId(), item.getCount(), 'sell') null, return false");
				}
				return false;
			}
		...
		for(ItemRequest item : items)
		{
			...
			if(oldItem == null){
				if(Config.DEBUG){
					LOG.info("[PrivateStoreSell] oldItem == null, return false");
				}
				return false;
			}
		...
		if(price > ownerInventory.getAdena())
		{
			if(Config.DEBUG)
			{
				LOG.info("[PrivateStoreSell] price > ownerInventory.getAdena(), return false");
			}
